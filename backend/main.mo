...
actor {
  ...
  var siteContent : SiteContent = {
    heroHeadline = "Welcome to Off-Grid Secrets";
    heroSubtitle = "Discover original art and insights for off-grid living";
    missionStatement = "Our mission is to inspire and empower individuals to embrace self-sufficiency and creativity.";
    contactEmail = "offgridsecrets@gmail.com";
  };
  ...
  // Admin functions
  public shared ({ caller }) func addPainting(
    id : PaintingId,
    title : Text,
    description : Text,
    price : Price,
  ...
  public shared ({ caller }) func updatePainting(
    id : PaintingId,
    updatedTitle : Text,
    updatedDescription : Text,
    updatedPrice : Price,
    updatedImages : [Storage.ExternalBlob]
  ) : async () {
    if (not (AccessControl.isAdmin(accessControlState, caller))) {
  ...
  public shared ({ caller }) func togglePaintingSoldStatus(id : PaintingId) : async () {
    if (not (AccessControl.isAdmin(accessControlState, caller))) {
      Runtime.trap("Unauthorized: Only admins can toggle sold status");
    };
    ...
    let updatedPainting : Painting = {
      existingPainting with sold = not existingPainting.sold
    };
